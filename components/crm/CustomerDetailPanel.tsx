...
import { X, Mail, Phone, MapPin, Calendar, Star, Ship, Package } from 'lucide-react';
...
export function CustomerDetailPanel({ customer, onClose }: CustomerDetailPanelProps) {
  ...
  const [activeTab, setActiveTab] = useState<'history' | 'communication' | 'notes' | 'surveys'>('history');
  ...
  const bookingHistory = [
    { id: 'BK-001', yacht: 'Azure Dream', package: 'Sunset Experience', date: 'Feb 15, 2026', amount: 3850, status: 'Completed' },
    { id: 'BK-002', yacht: 'Ocean Majesty', package: 'Full Day Charter', date: 'Jan 20, 2026', amount: 5200, status: 'Completed' },
    { id: 'BK-003', yacht: 'Twin Seas', package: 'Island Hopping', date: 'Dec 10, 2025', amount: 2400, status: 'Completed' }
  ];
  
  const communications = [
    { type: 'email', message: 'Booking confirmation sent for Azure Dream charter', timestamp: '2 hours ago', icon: Mail },
    { type: 'phone', message: 'Follow-up call regarding package upgrade options', timestamp: '1 day ago', icon: Phone },
    { type: 'email', message: 'Thank you email after completed booking', timestamp: '3 days ago', icon: Mail }
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 md:mb-6 pb-4 md:pb-6 border-b" style={{ borderColor: colors.cardBorder }}>
        <h3 className="text-lg md:text-xl font-bold" style={{ color: colors.textPrimary }}>
          Customer Details
        </h3>
        <button 
          onClick={onClose}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-4 md:gap-6">
        ...
        >
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            />
            <div className="text-lg md:text-xl font-bold mb-1 text-center" style={{ color: colors.textPrimary }}>
              {customer.name}
            </div>
            <div 
              className="px-3 py-1 rounded-full text-xs font-medium"
              style={{
                backgroundColor: customer.segment === 'VIP' ? `${colors.accentGold}15` : 
                                customer.segment === 'Regular' ? `${colors.accent}15` : `${colors.success}15`,
                color: customer.segment === 'VIP' ? colors.accentGold : 
                      customer.segment === 'Regular' ? colors.accent : colors.success
              }}
            >
          ...
          <div className="space-y-3 mb-6">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm break-all" style={{ color: colors.textPrimary }}>
                {customer.email}
              </span>
            </div>
            <div className="flex items-center gap-3">
          ...
          >
            <div className="text-xs uppercase tracking-wide mb-2" style={{ color: colors.textSecondary }}>
              Loyalty Tier
            </div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-lg font-bold" style={{ color: getLoyaltyColor(customerDetails.loyaltyTier) }}>
              ...
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star 
                    ...
                    className="w-4 h-4"
                    style={{ 
                      color: star <= Math.floor(customerDetails.satisfactionScore) ? colors.accentGold : colors.cardBorder,
                      fill: star <= Math.floor(customerDetails.satisfactionScore) ? colors.accentGold : 'none'
                    }}
                  />
                ))}
              </div>
          ...
          <div className="flex flex-wrap gap-2 mb-4 md:mb-6">
            {tabs.map((tab) => (
              <button
                ...
                style={{
                  backgroundColor: activeTab === tab.id ? `${colors.accent}15` : colors.background,
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <Ship className="w-5 h-5" style={{ color: colors.accent }} />
                          </div>
                          <div>
