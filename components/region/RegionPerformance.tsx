...
export function RegionPerformance({ regions }: RegionPerformanceProps) {
  ...
  const [metric, setMetric] = useState<'revenue' | 'bookings' | 'occupancy'>('revenue');
  
  const metrics = [
    { id: 'revenue' as const, label: 'Revenue', format: (val: number) => `$${(val / 1000).toFixed(0)}K` },
    { id: 'bookings' as const, label: 'Bookings', format: (val: number) => val.toString() },
    { id: 'occupancy' as const, label: 'Occupancy Rate', format: (val: number) => `${val}%` },
  ];
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        ...
        <div className="flex gap-1 p-1 rounded-lg w-fit" style={{ backgroundColor: colors.background }}>
          {metrics.map((m) => (
            <button
              key={m.id}
