...
import { createAgent, pricingKeys } from "@/lib/api/pricing";
...
export function AgentCreateDrawer({ isOpen, onClose }: AgentCreateDrawerProps) {
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.email || !form.regionId || !form.commissionRate) return;
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between h-14 px-4 border-b shrink-0" style={{ borderColor: colors.cardBorder }}>
          <h2 id="agent-create-title" className="text-lg font-semibold" style={{ color: colors.textPrimary }}>
            Add Agent
          </h2>
          <button
            type="button"
...
export default AgentCreateDrawer;
