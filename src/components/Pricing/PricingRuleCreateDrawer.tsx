...
import { createPricingRule, pricingKeys } from "@/lib/api/pricing";
import { useQueryClient } from "@tanstack/react-query";

interface PricingRuleCreateDrawerProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export function PricingRuleCreateDrawer({ isOpen, onClose }: PricingRuleCreateDrawerProps) {
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.ruleType || !form.startDate || !form.endDate) return;
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="pricing-rule-create-title"
      >
        <div
          className="flex items-center justify-between h-14 px-4 border-b shrink-0"
          style={{ borderColor: colors.cardBorder }}
        >
          <h2 id="pricing-rule-create-title" className="text-lg font-semibold" style={{ color: colors.textPrimary }}>
            Add Pricing Rule
          </h2>
          <button
            type="button"
...
export default PricingRuleCreateDrawer;
