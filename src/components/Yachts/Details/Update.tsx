...
const YachtsUpdate: React.FC<CustomerProps> = ({ goToPrevTab, id }) => {
  ...
  const tr = yachts?.yacht_translations?.find(t => t.locale === 'en') ?? yachts?.yacht_translations?.[0];
  const formik = useFormik<FormYachtsUpdateValues>({
    enableReinitialize: true,
    initialValues: {
      "Boat Type": yachts?.boat_type ?? "",
      Title: tr?.title ?? yachts?.name ?? "",
      Category: yachts?.price_category ?? "",
      Capacity: yachts?.capacity ?? "",
      Length: yachts?.length != null ? String(yachts.length) : "",
      "Length Range": yachts?.length_range ?? "",
      Cabins: yachts?.cabins != null ? String(yachts.cabins) : "",
      Bathrooms: yachts?.bathrooms != null ? String(yachts.bathrooms) : "",
      "Passenger Day Trip": yachts?.passenger_day_trip != null ? String(yachts.passenger_day_trip) : "",
      "Passenger Overnight": yachts?.passenger_overnight != null ? String(yachts.passenger_overnight) : "",
      Guests: yachts?.guests != null ? String(yachts.guests) : "",
      "Guests Range": yachts?.guests_range ?? "",
      "Day Trip Price": yachts?.day_trip_price != null ? String(yachts.day_trip_price) : "",
      "Overnight Price": yachts?.overnight_price != null ? String(yachts.overnight_price) : "",
      "Daytrip Price (Euro)": yachts?.daytrip_price_euro != null ? String(yachts.daytrip_price_euro) : "",
      "Primary Image": yachts?.primary_image ?? yachts?.yacht_gallery_images?.[0]?.image_url ?? "",
      "Gallery Images": convertExistingImages(
        yachts?.yacht_gallery_images?.map(img => img.image_url) ?? [],
      ),
      "Day Charter": tr?.day_charter ?? "",
      "Overnight Charter": tr?.overnight_charter ?? "",
      "About this Boat": tr?.about_this_boat ?? "",
      Specifications: tr?.specifications ?? "",
      "Boat Layout": tr?.boat_layout ?? "",
      "Video Link": yachts?.video_link ?? "",
      Badge: yachts?.badge ?? "",
      Slug: tr?.slug ?? "",
      Design: yachts?.design ?? "",
      Built: yachts?.built ?? "",
      "Cruising Speed": yachts?.cruising_speed ?? "",
      "Length Overall": yachts?.length_overall != null ? String(yachts.length_overall) : "",
      "Fuel Capacity": yachts?.fuel_capacity ?? "",
      "Water Capacity": yachts?.water_capacity ?? "",
      Code: yachts?.code ?? "",
      "Yacht Type": yachts?.type ?? "",
      "Tags": yachts?.yacht_tags?.map(t => t.tag) ?? [],
    },
