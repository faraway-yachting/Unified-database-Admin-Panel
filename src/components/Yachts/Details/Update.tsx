...
const YachtsUpdate: React.FC<CustomerProps> = ({ goToPrevTab, id }) => {
  ...
  const y = yachts as Record<string, unknown> | null | undefined;
  const formik = useFormik<FormYachtsUpdateValues>({
    enableReinitialize: true,
    initialValues: {
      "Boat Type": (y?.boatType as string) ?? yachts?.type ?? "",
      Title: yachts?.name ?? (y?.title as string) ?? "",
      Category: (y?.price as string) ?? "",
      Capacity: yachts?.capacityGuests != null ? String(yachts.capacityGuests) : (y?.capacity as string) ?? "",
      Length: yachts?.lengthM != null ? String(yachts.lengthM) : (y?.length as string) ?? "",
      "Length Range": (y?.lengthRange as string) ?? "",
      Cabins: (y?.cabins as string) ?? "",
      Bathrooms: (y?.bathrooms as string) ?? "",
      "Passenger Day Trip": (y?.passengerDayTrip as string) ?? "",
      "Passenger Overnight": (y?.passengerOvernight as string) ?? "",
      Guests: (y?.guests as string) ?? "",
      "Guests Range": (y?.guestsRange as string) ?? "",
      "Day Trip Price": (y?.dayTripPrice as string) ?? "",
      "Overnight Price": (y?.overnightPrice as string) ?? "",
      "Daytrip Price (Euro)": (y?.daytripPriceEuro as string) ?? "",
      "Primary Image": (yachts?.images?.find((i) => i.isCover)?.imageUrl ?? yachts?.images?.[0]?.imageUrl) ?? (y?.primaryImage as string) ?? "",
      "Gallery Images": convertExistingImages(
        (y?.galleryImages as string[] | undefined) ?? yachts?.images?.map((i) => i.imageUrl) ?? [],
      ),
      "Day Charter": (y?.dayCharter as string) ?? "",
      "Overnight Charter": (y?.overnightCharter as string) ?? "",
      "About this Boat": (y?.aboutThisBoat as string) ?? "",
      Specifications: (y?.specifications as string) ?? "",
      "Boat Layout": (y?.boatLayout as string) ?? "",
      "Video Link": (y?.videoLink as string) ?? "",
      Badge: (y?.badge as string) ?? "",
      Slug: (y?.slug as string) ?? "",
      Design: (y?.design as string) ?? "",
      Built: (y?.built as string) ?? "",
      "Cruising Speed": (y?.cruisingSpeed as string) ?? "",
      "Length Overall": (y?.lengthOverall as string) ?? "",
      "Fuel Capacity": (y?.fuelCapacity as string) ?? "",
      "Water Capacity": (y?.waterCapacity as string) ?? "",
      Code: (y?.code as string) ?? "",
      "Yacht Type": yachts?.type ?? (y?.type as string) ?? "",
      ...
      try {
        ...
        await updateYachtMutation.mutateAsync({
          payload: {
            ...
            dayCharter: values["Day Charter"] || (y?.dayCharter as string) || "",
            overnightCharter:
              values["Overnight Charter"] || (y?.overnightCharter as string) || "",
            aboutThisBoat:
              values["About this Boat"] || (y?.aboutThisBoat as string) || "",
            specifications:
              values["Specifications"] || (y?.specifications as string) || "",
            boatLayout: values["Boat Layout"] || (y?.boatLayout as string) || "",
